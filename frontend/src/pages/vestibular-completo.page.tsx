...
export const VestibularCompletoPage = () => {
  ...
  const handleResponderTodas = () => {
    ...
    questoesComReferencias.forEach((questao) => {
      ...
      console.log(
        `Questão ${questao.id} - alternativa selecionada:`,
  ...
  return (
    <div className="flex min-h-screen flex-col items-center gap-6 bg-gradient-to-b from-[#f5f7fa] to-[#c3cfe2] p-6">
      <button
        onClick={() => navigate("/")}
        className="mr-auto rounded-lg bg-green-200 px-4 py-2 font-medium text-gray-800 transition duration-200 hover:bg-green-300"
      >
        Voltar
      </button>
      ...
      {questoesComReferencias?.map((questao) => {
        ...
        return (
          <div
            key={questao.id}
            className={`w-full max-w-xl rounded-2xl p-6 shadow-xl transition-colors duration-300 ${
              respondida ? "bg-gray-300" : "bg-white"
            }`}
          >
            <div className="mb-4 w-fit rounded-full bg-gray-200 px-4 py-1 text-sm font-semibold text-gray-700">
              {questao.prova}
            </div>
            <div className="mb-4 rounded-md bg-gray-200 p-2 shadow-md">
            ...
            <h2
              className={`mb-4 text-lg font-bold ${
                respondida ? "text-gray-700" : "text-gray-800"
              }`}
            >
            ...
            <div className="flex flex-col gap-2">
              {questao.alternativas.map((alternativa: any) =>
                alternativa.id_questao === questao.id ? (
                  <label
                    key={alternativa.id}
                    className={`flex cursor-pointer items-center gap-2 rounded-md border px-4 py-2 transition-colors duration-200 ${
              ...
              resultado[questao.id] !== null && (
                <p
                  className={`mt-3 font-semibold ${
                    resultado[questao.id] ? "text-green-600" : "text-red-600"
                  }`}
                >
      ...
      <div className="flex w-full max-w-4xl flex-col gap-6 rounded-2xl bg-white p-8 shadow-xl">
        ...
        {redacao?.referencias?.map((referencia: any, index: number) => (
          <div
            key={index}
            className="mb-6 rounded-md bg-gray-200 p-2 shadow-md"
          >
            ...
            {referencia.data?.dados?.url_imagem ? (
              <img
                className="align-center mx-auto mt-4 flex justify-center"
        ...
        <div className="rounded-xl border-2 text-center shadow-md">
          <h1 className="text-xl leading-snug text-gray-800">
            A partir da coletânea apresentada, elabore um texto narrativo ou um
            texto dissertativo-argumentativo explorando o seguinte tema:
          </h1>
        ...
        <div className="text-center">
          <h1 className="text-xl font-bold">Orientações</h1> <br />
          ...
          <h1 className="mt-2 text-start text-xl font-bold">
          ...
          <p className="text-md text-start">Não o redija em versos;</p>
          <p className="text-md text-start">Organize-o em parágrafos;</p>
          <p className="text-md text-start">
        ...
        <button
          ...
          className={`mt-4 rounded-lg px-6 py-3 font-semibold shadow transition ${
            Object.keys(respostasSelecionadas).length !==
            questoesComReferencias.length
              ? "cursor-not-allowed bg-gray-400 text-white" // estado desabilitado (exemplo)
